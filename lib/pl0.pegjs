...
st     = CL s1:st? r:(SC st)* SC* CR {
               let t = [];
               if (s1) t.push(s1);
               return new Compound(location(), t.concat(r.map( ([_, st]) => st )))
            }
       / IF e:assign THEN st:st ELSE sf:st
           {
             return new IfThenElse(location(), e, st, sf);
           }
       / IF e:assign THEN st:st
           {
             return new IfThenElse(location(), e, st);
           }
       / WHILE a:assign DO st:st {
             return new While(location(), a, st);
           }
       / RETURN a:assign? {
             return new Return(location(), assign);
           }
       / assign

assign = i:ID ASSIGN e:cond {
              return new Assign(location(), i, e);
            }
       / cond

cond = l:exp op:COMP r:exp {
              return new Operation(location(), op, [l, r]);
            }
     / exp
