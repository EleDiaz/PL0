    ...
    = cD:constantDeclaration? vD:varDeclaration? fD:functionDeclaration* st:st
      {
        let constants = cD? cD : [];
        let variables = vD? vD : [];
        return new ADT.Block(variables, constants, fD, st)
      }
...
functionDeclaration "function"
    = FUNCTION id:ID LEFTPAR !COMMA p1:ID? r:(COMMA ID)* RIGHTPAR SC b:block SC
      {
        let params = p1? [p1] : [];
...
st "statements(st, if, while, return)"
    ...
    / RETURN a:assign?
      {
        return new ADT.Return(location(), assign);
      }
    / assign
